...
export default function useSelectedFilters() {
	const { setTempFilter, resetFilters, getTempFilters, filters } = useFilterStore();
	...
	// Sinkronisasikan label brand ketika store brand dikosongkan
	useEffect(() => {
		if (!filters.brand) {
			setSelectedFilters((prev) => ({ ...prev, brand: "Semua Brand" }));
		}
	}, [filters.brand]);
